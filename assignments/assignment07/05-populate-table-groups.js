...
var addressesForDb = JSON.parse(fs.readFileSync('data/parsed/parsed-groupTable.json'))

async.eachSeries(addressesForDb, function(value, callback) {
    const client = new Client(db_credentials);
    client.connect();
    console.log(value);
    var thisQuery = `INSERT INTO groups VALUES ('${parseInt(value.groupPK)}', '${value.name.replace("'", "")}', '${value.description.join(';')}', '${value.scheduleDetails.join(';')}');`;
